...
import sys
import time
def SCCN(opt_path,sar_path,save_path,noise_std,p_times,p_step,lam,t_times,t_step,info):
    ...
    sar4h=tf.sigmoid(tf.nn.conv2d(sar3h , sar4w ,strides=[1,1,1,1],padding='SAME')+sar4b)
    ...
    print(phi)
    step = t_step
    decay_rate = 0.96  # 琛板噺鐜�
    global_steps = t_times # 鎬荤殑杩唬娆℃暟
    decay_steps = 100  # 琛板噺娆℃暟
    global_ = tf.Variable(tf.constant(0))
    learning_rate = tf.compat.v1.train.exponential_decay(step,global_,decay_steps=global_steps,decay_rate=decay_rate,staircase=False)
